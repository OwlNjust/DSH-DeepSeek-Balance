...
export default {
  ...
  async apply(ctx) {
    ...
    async function fetchBalance() {
      ...
      if (json && json.error) throw new Error(String((json.error && json.error.message) || '余额接口错误'))
    ...
    function phaseInfo(nowMs) {
      ...
      const kinds = ['valley', 'peak', 'valley', 'peak', 'valley']
      const list = []
      for (let dayOff = -1; dayOff <= 1; dayOff++) {
        const dayS = dayStartMs + dayOff * 86400000
        for (let i = 0; i < 6; i++) list.push({ t: dayS + B[i] * 60000, kind: i < 5 ? kinds[i] : 'valley' })
      }
      list.sort((a, b) => a.t - b.t)
      let seg = null
      for (let i = 0; i < list.length - 1; i++) {
        if (nowMs >= list[i].t && nowMs < list[i + 1].t && list[i].kind) {
          seg = list[i]
          seg.end = list[i + 1].t
          break
        }
      ...
      return {
        ...
        endMs: seg.end,
        note: peak
          ? '高峰时段（北京时间 09:00-12:00、14:00-18:00）：价格是空闲时段的 2 倍，请减少 API 调用！'
          : '空闲时段（其余时间）：价格为高峰的 50%（半价），适合集中调用。',
      }
    }

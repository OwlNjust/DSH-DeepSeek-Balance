...
window.__ModuleLoader__.load({
  ...
  factory(require) {
    ...
    // Pill position persistence (per browser origin). The pill can be moved by
    // press-dragging with the LEFT or RIGHT mouse button; right-click on the pill
    // never opens the native context menu because the pill is a drag handle.
    ...
    function Widget() {
      ...
      // --- pill dragging (left or right mouse button, press-and-drag) ---
      const startDrag = (ev) => {
        if (ev.button !== 0 && ev.button !== 2) return
        const root = rootRef.current
        const r = root ? root.getBoundingClientRect() : { left: 0, top: 0, width: 0, height: 0 }
        dragRef.current = { active: true, sx: ev.clientX, sy: ev.clientY, bx: r.left, by: r.top, w: r.width, h: r.height, moved: false, suppress: false, lastPos: null }
        setDragging(true)
      }
      React.useEffect(() => {
        if (!dragging) return
        const onMove = (ev) => {
          ...
          if (!d.moved && Math.abs(dx) + Math.abs(dy) < 4) return // small jitter = plain click
          d.moved = true
        ...
        const onUp = () => {
          ...
          if (d && d.active) {
            ...
            if (d.moved) {
              d.suppress = true
              ctxSuppressRef.current = true
              if (d.lastPos) { try { localStorage.setItem(POS_KEY, JSON.stringify(d.lastPos)) } catch { /* storage unavailable */ } }
            }
          }
          setDragging(false)
        }
      ...
      const pillClick = () => {
        const d = dragRef.current
        if (d && d.suppress) { d.suppress = false; return } // click that ended a drag
        setOpen(true)
      }
      const pillProps = {
        className: 'dsb-pill',
